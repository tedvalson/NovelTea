...
, _scene(nullptr)
{
	...
	setBackgroundBrush(style.SceneBackgroundColor);
...
void FlowView::contextMenuEvent(QContextMenuEvent *event)
{
	...
	QPoint pos = event->pos() - QPoint(node.width(), node.height()) / 2;
...
void FlowView::deleteSelectedNodes()
{
	...
	for (QGraphicsItem *item : _scene->selectedItems()) {
		if (auto c =
				qgraphicsitem_cast<ConnectionGraphicsObject *>(item))
	...
	// otherwise
	// qgraphicsitem_cast<NodeGraphicsObject*>(item) could be a
	// use-after-free
...
void FlowView::mouseMoveEvent(QMouseEvent *event)
{
	...
	if (scene()->mouseGrabberItem() == nullptr &&
		...
		if ((event->modifiers() & Qt::ShiftModifier) == 0) {
			QPointF difference =
				_clickPos - mapToScene(event->pos());
			setSceneRect(sceneRect().translated(difference.x(),
												difference.y()));
		}
	}
}
...
void FlowView::drawBackground(QPainter *painter, const QRectF &r)
{
	...
	QPen pfine(style.SceneFineGridColor, 1.0);

	painter->setPen(pfine);
	drawGrid(15);

	QPen p(style.SceneCoarseGridColor, 1.0);
