...
void Map::loadJson(const json &j)
{
	...
	for (auto &jroom : j[5].ArrayRange()) {
		...
		IntRect rect(jroom[1].ToInt(), jroom[2].ToInt(), jroom[3].ToInt(), jroom[4].ToInt());
	...
	for (auto &jconn : j[6].ArrayRange()) {
		...
		addConnection(jconn[0].ToInt(), jconn[1].ToInt(),
			Vector2i(jconn[2].ToInt(), jconn[3].ToInt()),
			Vector2i(jconn[4].ToInt(), jconn[5].ToInt()),
...
int Map::addRoom(const std::string &name, const IntRect &rect, const std::vector<std::string> &roomIds, const std::string &script, RoomStyle style)
{
...
void Map::addConnection(int roomStart, int roomEnd, const Vector2i &portStart, const Vector2i &portEnd, const std::string &script, ConnectionStyle style)
{
...
bool Map::checkForDoor(const MapConnection &c, FloatRect &doorRect) const
{
	bool isDoor = false;
	auto& rectStart = m_rooms[c.roomStart]->rect;
	auto& rectEnd = m_rooms[c.roomEnd]->rect;
	Vector2i pStart(rectStart.left + c.portStart.x, rectStart.top + c.portStart.y);
	Vector2i pEnd(rectEnd.left + c.portEnd.x, rectEnd.top + c.portEnd.y);
	doorRect = FloatRect(std::min(pStart.x, pEnd.x),
