...
void NinePatch::setContentSize(const sf::Vector2f &size) const
{
	m_contentSize = size;
	m_needsUpdate = true;
}
...
{
	setContentSize(sf::Vector2f(width, height));
}


void NinePatch::setSize(const sf::Vector2f &size) const
{
...
const sf::Vector2f &NinePatch::getContentSize() const
{
	ensureUpdate();
...
void NinePatch::setPadding(const sf::FloatRect &padding)
{
	m_needsUpdate = true;
	m_padding = padding;
}
...
{
	setPadding(sf::FloatRect(left, top, width, height));
}


const sf::FloatRect &NinePatch::getPadding() const
{
	ensureUpdate();
	return m_padding;
}
...
void NinePatch::setColor(const sf::Color &color) const
{
	m_color = color;
	m_needsUpdate = true;
}
...
void NinePatch::updateRegions() const
{
	...
	if (!m_texture)
	{
		...
		region.rect = sf::FloatRect(0.f, 0.f, 1.f, 1.f);
	...
	{
		sf::Color color_y = image.getPixel(0, y);
		bool is_region_y = is_static_y ? (color_y == sf::Color::Black) : (color_y.a == 0);
		...
		if (is_region_y)
		{
			...
			{
				sf::Color color_x = image.getPixel(x, 0);
				bool is_region_x = is_static_x ? (color_x == sf::Color::Black) : (color_x.a == 0);
	...
	for (y = 1; y < image.getSize().y - 1; y++)
	{
		sf::Color color = image.getPixel(image.getSize().x - 1, y);
		if (is_padding) {
			if (color.a == 0) {
	...
	for (x = 1; x < image.getSize().x - 1; x++)
	{
		sf::Color color = image.getPixel(x, image.getSize().y - 1);
		if (is_padding) {
			if (color.a == 0) {
	...
	for (const auto &region: m_regions)
	{
		...
		switch (region.type) {
			...
			default:
				addedWidth = false;
				addedHeight = false;
				vertices[0].position = sf::Vector2f(region.rect.left+dynamicWidthSum-1, region.rect.top+dynamicHeightSum-1);
				vertices[1].position = sf::Vector2f(region.rect.left+dynamicWidthSum-1 + region.rect.width, region.rect.top+dynamicHeightSum-1);
				vertices[2].position = sf::Vector2f(region.rect.left+dynamicWidthSum-1, region.rect.top+dynamicHeightSum-1 + region.rect.height);
				break;
		}
